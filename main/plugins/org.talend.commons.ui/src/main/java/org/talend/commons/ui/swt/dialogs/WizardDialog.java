...
public class WizardDialog extends TitleAreaDialog implements IWizardContainer2, IPageChangeProvider {
    ...
    protected int pageMessageType = IMessageProvider.NONE;
    ...
    public WizardDialog(Shell parentShell, IWizard newWizard) {
        ...
        setShellStyle(
                SWT.CLOSE | SWT.MAX | SWT.TITLE | SWT.BORDER | SWT.RESIZE | getShellModality(modal) | getDefaultOrientation());
        setWizard(newWizard);
    ...
    /**
     * Option to set the modality of the WizardDialog. This method must be called before the dialog's shell is created,
     * e.g. before you call {@link #open()} for the first time.
     *
     * @param modal true (default) if the WizardDialog should block the underlying window.
    ...
    /**
     * @return <code>false</code> if the user interface blocks the underlying window (modal) or <code>true</code> if the
     * underlying window is not blocked.
     * @since 3.16
     * @see #setModal(boolean)
     ...
     * <p>
     * The <code>WizardDialog</code> implementation of this framework method prevents the parent composite's columns
     * from being made equal width in order to remove the margin between the Back and Next buttons.
     * </p>
     *
    ...
    protected ProgressMonitorPart createProgressMonitorPart(Composite composite, GridLayout pmlayout) {
        ...
        return new ProgressMonitorPart(composite, pmlayout, true) {
            ...
            protected void initialize(Layout layout, int progressIndicatorHeight) {
                ...
                this.setForeground(composite.getForeground());
                this.setBackground(composite.getBackground());
                this.fLabel.setForeground(composite.getForeground());
                this.fLabel.setBackground(composite.getBackground());
                this.fProgressIndicator.setForeground(composite.getForeground());
                this.fProgressIndicator.setBackground(composite.getBackground());
            }
    ...
    private MessageDialog createWizardClosingDialog() {
        return new MessageDialog(getShell(), JFaceResources.getString("WizardClosingDialog.title"), //$NON-NLS-1$
                null, JFaceResources.getString("WizardClosingDialog.message"), //$NON-NLS-1$
                MessageDialog.QUESTION, 0, IDialogConstants.OK_LABEL) {
            @Override
     ...
     * @param saveState a map containing the enabled/disabled state of the wizard dialog's buttons
     * @param key the key
     ...
     * showing page.
     *
     * @param saveState a map containing the saved state as returned by <code>saveUIState</code>
     * @see #saveUIState
     */
     ...
     *
     * @param control the control, or <code>null</code> if none
     * @param saveState a map containing the enabled/disabled state of the wizard dialog's buttons
     * @param key the key
    ...
    private void updateForPage(IWizardPage page) {
        ...
        if (page.getControl() == null) {
            ...
            Assert.isNotNull(page.getControl(), JFaceResources.format(JFaceResources.getString("WizardDialog.missingSetControl"), //$NON-NLS-1$
                    page.getName()));
    ...
    /**
     * Computes the correct dialog size for the current page and resizes its shell if necessary. Also causes the
    ...
    /**
     * Notifies any selection changed listeners that the selected page has changed. Only listeners registered at the
    ...
    /**
     * Notifies any page changing listeners that the currently selected dialog page is changing. Only listeners
